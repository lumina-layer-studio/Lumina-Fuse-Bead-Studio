...
describe("Workshop theme contract", () => {
  ...
  it("keeps the bead editor in a responsive 3D-first workspace without permanent scroll docks", async () => {
    ...
    expect(floatingWorkspace).toMatch(
      /\.bead-editor-workspace__canvas\s*{[\s\S]*?position:\s*absolute;[\s\S]*?inset:\s*0;[\s\S]*?\.bead-editor-workspace__canvas\s*>\s*\.canvas-stage\s*{[\s\S]*?padding:\s*76px\s*16px\s*164px\s*88px;/,
    );
    expect(floatingWorkspace).toMatch(
    ...
    expect(css).toMatch(
      /\.palette-swatch__bead\s*{[^}]*width:\s*48px;[^}]*height:\s*43px;/,
    );
    expect(floatingWorkspace).toMatch(
      /\.bead-editor-palette-strip__body\s*{[^}]*display:\s*flex;[^}]*min-width:\s*0;[^}]*gap:\s*8px;/,
    );
    expect(floatingWorkspace).toMatch(
      /\.bead-editor-palette-add\s*{[^}]*flex:\s*0\s+0\s+auto;/,
    );
    expect(floatingWorkspace).toMatch(
